...
Download_ocserv(){
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/smlemk3/ocs_erv_cer/master/ocserv.conf"
...
Service_ocserv(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/smlemk3/ocs_erv_cer/master/ocserv_debian -O /etc/init.d/ocserv; then
...
tls_www_server' > server.tmpl
	...
	cp ca-cert.pem /etc/ocserv/ssl/ca-cert.pem
	cp ca-key.pem /etc/ocserv/ssl/ca-key.pem
...
Installation_dependency(){
	...
	if [[ ${release} = "centos" ]]; then
		...
		if [[ $? = 0 ]]; then
			...
			wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/smlemk3/ocs_erv_cer/master/us.sources.list"
			apt-get update
			apt-get install vim net-tools pkg-config build-essential libgnutls28-dev libwrap0-dev liblz4-dev libseccomp-dev libreadline-dev libnl-nf-3-dev libev-dev gnutls-bin -y
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/smlemk3/ocs_erv_cer/master/ocserv.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ocserv" ]]; then
		rm -rf /etc/init.d/ocserv
		Service_ocserv
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/smlemk3/ocs_erv_cer/master/ocserv.sh" && chmod +x ocserv.sh
